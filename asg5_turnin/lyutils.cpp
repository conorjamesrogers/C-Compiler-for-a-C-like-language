...
void lexer::newfilename (const string& filename) {
   ...
   // printf("filenames vector size: %zd\n\n", lexer::filenames.size());
...
void lexer::include() {
   ...
   if (scan_rc != 2) {
      ...
      // fprintf (fopen("filelist", "a"), "# %zd \"%s\"\n",
      //             linenr, filename);
      // fclose("filelist.tmp");
      
      char file_tmp[0x1000];
      sprintf(file_tmp,"# %zd \"%s\"\n", linenr, filename);
      string files_str_tmp(file_tmp);
      lexer::filenames_print += files_str_tmp;
...
string* lexer::get_filenames_print()
{
   return &lexer::filenames_print;
}
